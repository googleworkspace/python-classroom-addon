...
def load_activity_attachment():
    ...
    if form.validate_on_submit():
        ...
        if not SET_GRADE_WITH_LOGGED_IN_USER_CREDENTIALS:
            ...
            # https://developers.google.com/classroom/eap/add-ons-alpha/walkthroughs/grade-passback
            grade = 0

            # See if the student response matches the stored name.
            if form.student_response.data.lower(
            ...
            # for this attachment.
            patch_grade_response = teacher_classroom_service.courses().posts(
...
def view_submission():
    ...
    if SET_GRADE_WITH_LOGGED_IN_USER_CREDENTIALS:
        ...
        patch_grade_response = classroom_service.courses().posts(
        ).addOnAttachments().studentSubmissions().patch(
            courseId=flask.session["courseId"],
            postId=flask.session["postId"],
            attachmentId=flask.session["attachmentId"],
